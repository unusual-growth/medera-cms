...
@php
    ...
    if (config('twill.enabled.permissions-management')) {
        ...
        $groups = app()
            ->make('A17\Twill\Repositories\GroupRepository')
            ->get()
            ->map(function ($group) {
                return [
                            ...
                            <div class="publisher" data-sticky-target="publisher">
                                ...
                                @else
                                    @hasSection('sideFieldset')
                                        <x-twill::formFieldset id="options"
                        ...
                        <section class="col col--primary" data-sticky-top="publisher">
                            ...
                            @else
                                @unless ($disableContentFieldset)
                                    <x-twill::formFieldset id="content"
                                    ...
                                    @can('manage-item', isset($item) ? $item : null)
                                        <x-twill::formFieldset id="permissions" title="User Permissions" :open="false">
                                            <x-twill::select-permissions :items-in-selects-tables="$users" label-key="name"
                                                name-pattern="user_%id%_permission" :list-user="true" />
                                        </x-twill::formFieldset>
                                    @endcan
                                @endif
                            @endif
...
@section('initialStore')
    ...
    @if (isset($item) && classHasTrait($item, \A17\Twill\Models\Behaviors\HasMedias::class))
        window['{{ config('twill.js_namespace') }}'].STORE.medias.crops = {!! json_encode(
            $item->getMediasParams() +
                \A17\Twill\Facades\TwillBlocks::getAllCropConfigs(true) +
                (config('twill.settings.crops') ?? []),
        ) !!}
    @else
        window['{{ config('twill.js_namespace') }}'].STORE.medias.crops = {!! json_encode(
            \A17\Twill\Facades\TwillBlocks::getAllCropConfigs(true) + (config('twill.settings.crops') ?? []),
        ) !!}
    @endif
                ...
                if (!isNaN(groupId)) {
                    ...
                    users.forEach(function(userId) {
                        ...
                        if (currentPermission === '' || currentPermission === 'view-item') {
                            ...
                            window['{{ config('twill.js_namespace') }}'].vm.$store.commit(
